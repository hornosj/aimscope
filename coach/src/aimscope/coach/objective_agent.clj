...
(defn ^{:action/pre  ["objetivo/texto-carregado?"]
  ...
  (let [texto (g pc "objetivo/texto" "")
        ...
        prompt (str
                ...
                "sem nenhum texto fora do JSON. Os campos são eixos INDEPENDENTES "
                "(ex.: melhorar no Valorant SEM trocar de sens = sens-policy fixed "
                "+ game valorant).\n"
                "Campos:\n"
                "- \"sens-policy\": \"fixed\" (já tem uma sens, não quer mexer), "
                "\"range\" (aceita trocar dentro de um range) ou \"search\" "
                "(procura a melhor sens possível pra si).\n"
                "- \"game\": \"valorant\", \"cs2\", \"overwatch\", \"apex\" ou "
                "\"geral\" (geral = treina KovaaK's por si só).\n"
